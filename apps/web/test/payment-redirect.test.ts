...
describe('assertSafePaymentRedirect', () => {
  describe('HTTPS', () => {
    it('accepts a sandbox HTTPS provider URL in production', () => {
      const url = assertSafePaymentRedirect(
        'https://sandbox-provider.example/pay',
        'production',
      );
  ...
  describe('HTTP loopback (development and test only)', () => {
    it('accepts http://127.0.0.1:3090/momo-test/pay in development', () => {
      const url = assertSafePaymentRedirect(
        'http://127.0.0.1:3090/momo-test/pay',
        'development',
      );
    ...
    it('accepts http://localhost:3090/vnpay-test/pay in test', () => {
      const url = assertSafePaymentRedirect(
        'http://localhost:3090/vnpay-test/pay',
        'test',
      );
  ...
  describe('rejections', () => {
    it('rejects an external HTTP host in development', () => {
      expect(() =>
        assertSafePaymentRedirect('http://evil.example/pay', 'development'),
      ).toThrow();
    });
    ...
    it('rejects javascript: URLs', () => {
      expect(() =>
        assertSafePaymentRedirect('javascript:alert(1)', 'development'),
      ).toThrow();
    });
    ...
    it('rejects HTTP loopback in production', () => {
      expect(() =>
        assertSafePaymentRedirect('http://127.0.0.1:3090/pay', 'production'),
      ).toThrow();
    });
    ...
    it('rejects malformed URLs', () => {
      expect(() =>
        assertSafePaymentRedirect('not a real url at all', 'development'),
      ).toThrow();
    });
  });
});
