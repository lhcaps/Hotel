...
import { presentPhysicalRooms, roomStartingPrice } from '../../content/peace-home-physical-rooms';
...
export default async function PublicRoomsPage({
  ...
  const [cookieStore, catalog, resolvedSearchParams] = await Promise.all([
    cookies(),
    ...
    <main className="rooms-catalog" id="main-content">
      ...
      {state.kind === 'ready' ? (
        <section aria-label={translate(locale, 'catalog.list')} className="room-catalog-list">
          {presentPhysicalRooms(state.catalog)
            .filter((room) => {
            ...
            .map((room, index) => {
              ...
              return (
                <article
                  className="room-catalog-list__item"
                  key={room.slug}
