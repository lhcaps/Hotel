...
export type MomoCreate = {
  accessKey: string; amount: number; extraData: string; ipnUrl: string;
...
export type MomoIpn = {
  ...
  payType: string; requestId: string; responseTime: number; resultCode: number;
...
export function oracleMomoCreate(f: MomoCreate): string {
  return join([
    `accessKey=${f.accessKey}`, `amount=${f.amount}`, `extraData=${f.extraData}`,
...
export function oracleMomoQuery(f: MomoIpn): string {
  return join([
    `accessKey=${f.accessKey}`, `orderId=${f.orderId}`, `partnerCode=${f.partnerCode}`,
