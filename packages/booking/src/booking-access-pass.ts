...
const ARRIVAL_PACKAGE_LEAD_MS = 30 * 60 * 1000;

/** Customer-facing QR access is deliberately unavailable until T-30. */
export function isAccessPassWithinArrivalWindow(checkIn: Date, now: Date): boolean {
  return now.getTime() >= checkIn.getTime() - ARRIVAL_PACKAGE_LEAD_MS;
}

/**
 * Issues the same signed, short-lived pass for API and email delivery. The QR
 * payload contains no access configuration: it authorizes a server-side lookup.
 */
export class BookingAccessPassService {
  ...
  public toPng(pass: string): Promise<Buffer> {
    return QRCode.toBuffer(pass, {
      type: 'png',
      errorCorrectionLevel: 'M',
      margin: 2,
      width: 256,
