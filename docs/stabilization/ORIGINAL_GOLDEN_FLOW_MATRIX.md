...
| Derive ready room for next booking           | PARTIAL     | PARTIAL        | NO      | NO         | Client-side room grouping is not authoritative derived state; multi-property actor routing now implemented |
| Allocate next booking without overlap        | YES         | YES            | PARTIAL | PARTIAL    | Must be proven after the new readiness/housekeeping flow; property-authorization boundary now closed       |
